(function ($) {
    ...
    $('#price-range').slider({
        ...
        slide: function (event, ui) {
            ...
            $('#getprice-amount2').val(ui.values[1]);

        }
    });
    $('#price-amount').val('$' + $('#price-range').slider('values', 0) +
        '  -  $' + $('#price-range').slider('values', 1));
    $('#getprice-amount1').val('$' + $('#price-range').slider('values', 0));
    $('#getprice-amount2').val('$' + $('#price-range').slider('values', 1));
    //    slide: function( event, ui ) {
    //     $('#price-amount1').html( '<span>' + '$' + ui.values[ 0 ] + '</span>');
    //     $('#price-amount2').html( '<span>' + '$' + ui.values[ 1 ] + '</span>');
    //   }
    // });
    // $('#price-amount1').html( '<span>' + '$' + $( "#price-range" ).slider( "values", 0 ) + '</span>' );
    // $('#price-amount2').html( '<span>' + '$' + $( "#price-range" ).slider( "values", 1 ) + '</span>' );
    ...
    $('.qtybtn').on('click', function () {
        ...
        function updateSoLuong(newSL) {
            var idsp = $('#id_sp').val();
            var color = $('#selectedColorInput').val();
            var size = $('#selectedSizeInput').val();
            if (color == '' && size == '') {
                alert('Vui Lòng Chọn Màu Săc ')
                $input.val() = 1;
            } else if (color == '') {
                alert('Vui Lòng Chọn Màu Săc')
                $input.val() = 1;
                ...
                $.ajax({
                    type: 'POST',
                    url: 'view/addToCart.php?act=checkSl',
                    data: {
                        newQuantity: newSL,
                        mau: color,
                        kichco: size,
                    ...
                    success: function (response) {
                        if (response === 'valid') {
                            $input.val(newSL);
                            // $input.val(newVal);
                        } else if (response === 'saisl') {
                            alert('Sản Phẩm Đang hết hàng trong kho');
