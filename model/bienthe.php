<?php 
    function loadAll_mau(){
        $sql ="SELECT giatri_thuotinh.id, thuoc_tinh.name 'ten_thuoc_tinh', giatri_thuotinh.name 'ten_gia_tri_tt', giatri_thuotinh.id_thuoctinh,ma_mau 
                FROM thuoc_tinh JOIN giatri_thuotinh ON thuoc_tinh.id = giatri_thuotinh.id_thuoctinh 
                WHERE giatri_thuotinh.id_thuoctinh = 1
                ";
    ...
    function check_size($id_sp, $id_color, $id_size){
        $sql = "SELECT * FROM thong_tin_sp WHERE id_sp =$id_sp AND id_color = $id_color and id_size=$id_size";
        return pdo_query_one($sql);
    }

    function checkSl($id_sp,$mau, $size){
        $sql = "SELECT * FROM `thong_tin_sp` WHERE id_sp = $id_sp AND id_color = $mau AND id_size=$size ";
        return pdo_query_one($sql);
    }
?>